...
class PywolfException(Exception):
    ...
    class InternalException(Exception):
        ...
        class EntrypointException(Exception):
            def __init__ (self,name,import_from):
                self.entpoint=name
                self.import_from=import_from
            def __str__(self):
                if self.import_from=="__main__":
                    return f"{self.entpoint}は直接呼び出しには対応していません。"
                else:
                    return f"{self.entpoint}は{self.import_from}からの呼び出しには対応していません。直接呼び出してください。"
