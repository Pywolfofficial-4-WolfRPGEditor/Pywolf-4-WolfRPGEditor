...
imports.os.makedirs(funcs.joinpath(systempath["userdirroot"],"guest"),exist_ok=True)
title = f"PyWolf Launcher v{funcs.pywolfversion()} on {systempath['Pywolfroot']}"
if __name__ == "__main__":
    manifestpath=funcs.joinpath(systempath["systemassetroot"],"manifest","game")
    imports.os.makedirs(manifestpath,exist_ok=True)
    ...
    imports.logging.shutdown()
    imports.shutil.rmtree(funcs.joinpath(systempath["systemassetroot"],"temp"), ignore_errors=True)
    imports.shutil.rmtree(funcs.joinpath(systempath["userdirroot"],"guest"), ignore_errors=True)
    imports.sys.exit()
else:
    raise imports.Exception.PywolfException.InternalException.EntrypointException(f"{imports.sys.executable}",__name__)