        ...
        case "globalnetwork":
            ...
            while sorce is None:
                ...
                if sorcetemp[0]=="" or sorcetemp[1]==False:
                    ...
                    sorce=sorcetemp[0]
            resp=imports.requests.get(sorce)
            filebin=resp.content
            imports.os.makedirs(normpath(joinpath(systempath["autodownload"])),exist_ok=True)
            with open(normpath(joinpath(systempath["autodownload"],"autodownload.zip")),"+wb") as result:
                result.write(filebin)
...
def saveactionexec(window,systempath,userpath,game,action="import",managed=False):
    updatemanifest(window,systempath,imports.os.listdir(userpath["usergameroot"]))
    if managed:
...
def playSE(file,loop=True,window=None):
    ...
    effect.play()
def updatemanifest(window,systempath,gamelist):
    base="https://raw.githubusercontent.com/Pywolfofficial-4-WolfRPGEditor/Pywolf-WolfRPGEditor/refs/heads/main/manifest/game/"
    ...
    if len(gamelist)!=len(manifestlist):
        ...
        for game in gamelist:
            ...
            a=imports.requests.get(path)
            if a.status_code==404:
                with open(joinpath(manifestpath,game+".json"),"w") as b:
                    b.write("")
            else:
                with open(joinpath(manifestpath,game+".json"),"w") as b:
                    b.write(a.text)
